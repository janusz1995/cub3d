...
static	void	horz_x(t_data *img)
{
			...
			img->player.ang_start > M_PI_2)
	{
		if (img->player.ang_start < (M_PI + M_PI_2) &&
			...
			img->horz_x = img->player.x - img->calc.dis_y *
					tan((M_PI + M_PI_2) - img->player.ang_start);
		else
			img->horz_x = img->player.x - img->calc.dis_y *
					tan(img->player.ang_start - M_PI_2);
	}
	...
	else
	{
		if (img->player.ang_start > 0 && img->player.ang_start < M_PI_2)
			img->horz_x = img->calc.dis_y * tan(M_PI_2 - img->player.ang_start)
					+ img->player.x;
		else
			img->horz_x = img->calc.dis_y * tan(img->player.ang_start -
							(M_PI + M_PI_2)) + img->player.x;
	}
}
...
void			horz_x_and_y(t_data *img)
{
	if (img->player.ang_start > M_PI && img->player.ang_start < (M_PI * 2))
		img->horz_y = (int)(img->player.y / img->cube_size) *
