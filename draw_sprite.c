...
void 	draw_sprite(t_data *img, t_sprites *tmp)
{
	...
	img->calc.sprite_x = img->calc.sprite_dir * (img->width/2) / (M_PI / 6) +
							img->width/2 - sprite_h/2;
	...
	while (y < sprite_h)
	{
		...
		while (x < sprite_h)
		{
			if (img->calc.sprite_x + y < img->width && y + img->calc.sprite_x >= 0 &&
				img->calc.arr_min_len_wall[(int)(img->calc.sprite_x + y)] > tmp->len)
